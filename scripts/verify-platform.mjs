...
const originalOffer=JSON.parse(sqlite.prepare("SELECT data FROM entities WHERE id='voucher-example'").get().data);
const addOffer=async id=>call('/admin/entity',{...originalOffer,id,title:'Staff redemption test',expires:'2099-12-31'},'owner');
await addOffer('self-use');
const ownClaim=await call('/claim',{id:'self-use'},'member');
assert.equal((await call('/wallet',undefined,'member')).claims.find(c=>c.id===ownClaim.id).redeemable,true);
...
assert.equal(used.claim.status,'redeemed');assert.equal(used.claim.receipt.code,ownClaim.code);
assert.equal(used.claim.receipt.title,'Staff redemption test');assert.ok(used.claim.receipt.memberName);
assert.ok(used.serverTime);assert.equal(used.claim.redeemable,false);
await call('/redeem',{id:ownClaim.id,confirm:true},'member',409);
await call('/admin/redeem',{code:ownClaim.code},'owner',409);
await call('/admin/entity',{...originalOffer,id:'self-use',title:'Changed after redemption'},'owner');
await call('/admin/delete',{id:'self-use'},'owner');
assert.deepEqual((await call('/wallet',undefined,'member')).claims.find(c=>c.id===ownClaim.id).receipt,used.claim.receipt);
await addOffer('race-use');const raceClaim=await call('/claim',{id:'race-use'},'member');
const raceRequest=()=>new Request('https://slk.test/api/redeem',{method:'POST',headers:{origin:'https://slk.test','content-type':'application/json','oai-authenticated-user-id':'member'},body:JSON.stringify({id:raceClaim.id,confirm:true})});
...
assert.equal(sqlite.prepare("SELECT count(*) n FROM audit WHERE action='voucher.redeemed' AND target=?").get(raceClaim.id).n,1);
for(const reason of ['expired','archived','inactive']){
 await addOffer(reason);const claim=await call('/claim',{id:reason},'member');
 if(reason==='expired')await call('/admin/entity',{...originalOffer,id:reason,expires:'2000-01-01'},'owner');
 if(reason==='archived')await call('/admin/delete',{id:reason},'owner');
 if(reason==='inactive')sqlite.prepare("UPDATE users SET membership='inactive' WHERE id='member'").run();
 const checked=(await call('/wallet',undefined,'member')).claims.find(c=>c.id===claim.id);assert.equal(checked.redeemable,false);assert.ok(checked.invalidReason);
 await call('/redeem',{id:claim.id,confirm:true},'member',409);
 assert.equal(sqlite.prepare('SELECT status FROM claims WHERE id=?').get(claim.id).status,'available');
 if(reason==='inactive')sqlite.prepare("UPDATE users SET membership='active' WHERE id='member'").run();
}
console.log('PASS: claim → wallet → staff-confirmed redemption, private ownership, immutable receipts, duplicate/concurrent redemption protection and expired/archived/inactive restrictions.');
...
const attendee={id:'event-network',name:'Member Example',email:'attendee@example.test',phone:'0821234567',business:'Local business',consent:true};
...
assert.equal(interest.status,'pending_confirmation');assert.equal(interest.details.phone,'0821234567');
assert.equal(interest.details.banking.account,'63135151221');
assert.equal((await call('/registration?event=event-network',undefined,'outsider')).registration,null);
...
assert.equal((await call('/registration?event=event-network',undefined,'member')).registration.status,'confirmed');
const eventData=JSON.parse(sqlite.prepare("SELECT data FROM entities WHERE id='event-network'").get().data);
await call('/admin/entity',{...eventData,id:'paid-event',price:175},'owner');
const paid=(await call('/register',{...attendee,id:'paid-event',amount:1,status:'confirmed'},'member')).registration;
assert.equal(paid.details.amount,175);assert.equal(paid.status,'pending_payment');
...
assert.equal((await call('/registration?event=paid-event',undefined,'member')).registration.status,'payment_review');
...
assert.equal((await call('/registration?event=paid-event',undefined,'member')).registration.status,'confirmed');
// The server snapshots the redemption window and enforces it for members and staff.
await call('/admin/entity',{...originalOffer,id:'window-default',expires:'2099-12-31'},'owner');
...
assert.equal(Date.parse(savedDefault.redeem_by)-Date.parse(savedDefault.created_at),48*3600000);
await call('/admin/entity',{...originalOffer,id:'window-custom',expires:'2099-12-31',redeemHours:6},'owner');
...
await call('/admin/entity',{...originalOffer,id:'window-custom',expires:'2099-12-31',redeemHours:72},'owner');
assert.equal(sqlite.prepare('SELECT redeem_by FROM claims WHERE id=?').get(customWindow.id).redeem_by,savedCustom.redeem_by);
for(const redeemHours of [0,-1,1.5,'oops',9000])await call('/admin/entity',{...originalOffer,id:'window-invalid',redeemHours},'owner',400);
sqlite.prepare('UPDATE claims SET redeem_by=? WHERE id=?').run(new Date(Date.now()-1000).toISOString(),customWindow.id);
assert.equal((await call('/wallet',undefined,'member')).claims.find(c=>c.id===customWindow.id).redeemable,false);
...
assert.equal((await call('/wallet',undefined,'member')).claims.find(c=>c.id===defaultWindow.id).status,'redeemed');
await call('/admin/entity',{...originalOffer,id:'legacy-window',expires:'2099-12-31'},'owner');
const legacyWindow=await call('/claim',{id:'legacy-window'},'member');
sqlite.prepare('UPDATE claims SET redeem_by=NULL,created_at=? WHERE id=?').run(new Date(Date.now()-49*3600000).toISOString(),legacyWindow.id);
const legacyView=(await call('/wallet',undefined,'member')).claims.find(c=>c.id===legacyWindow.id);
assert.ok(legacyView.redeem_by);assert.equal(legacyView.redeemable,false);
await call('/redeem',{id:legacyWindow.id,confirm:true},'member',409);
await call('/admin/redeem',{code:legacyWindow.code},'owner',409);
console.log('PASS: event contact/payment workflow and configurable voucher deadlines, ownership, immutable claim deadlines and staff/member expiry enforcement.');
