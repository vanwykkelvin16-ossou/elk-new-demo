...
function ContactSection() {
  const [viewing, setViewing] = useState<ContactPerson | null>(null);
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!data) return null;
      const row = data as unknown as { general_email: string | null; contacts: unknown };
      return { general_email: row.general_email, contacts: parseContacts(row.contacts) };
    },
  });

  const contacts = (data?.contacts ?? []).filter(isMeaningfulContact);
  if (isLoading || (!data?.general_email && contacts.length === 0)) return null;
    ...
    <section>
      ...
      <Card className="overflow-hidden divide-y divide-border p-0">
        {data?.general_email && (
          ...
          >
            <div className="size-11 rounded-xl bg-primary text-primary-foreground grid place-items-center shrink-0 shadow-sm shadow-primary/20">
        ...
        {contacts.map((c, i) => (
          <div key={i} className="flex items-center gap-3.5 px-4 py-3.5">
            ...
            <button
              ...
              aria-label={c.image_url ? `View ${c.name}'s photo` : c.name}
              className="size-11 rounded-full overflow-hidden bg-primary/10 text-primary grid place-items-center shrink-0 font-bold text-sm ring-1 ring-border transition-transform enabled:hover:scale-105 enabled:active:scale-95 disabled:cursor-default"
            >
              {c.image_url ? (
            ...
            <a
              ...
              className="min-w-0 flex-1 flex items-center gap-2 group -my-3.5 py-3.5"
            >
              <span className="min-w-0 flex-1">
                <span className="block font-semibold text-sm leading-tight truncate group-hover:text-primary transition-colors">
                  {c.name || "SLK team"}
                </span>
              ...
              <Phone className="size-4 text-muted-foreground shrink-0 group-hover:text-primary transition-colors" />
            </a>
          </div>
        ))}
        ...
        <DialogContent className="max-w-xs rounded-3xl p-6 text-center">
          ...
          {viewing?.image_url && (
            <img
              ...
              alt={viewing.name}
              className="size-56 rounded-full object-cover mx-auto ring-4 ring-primary/15 shadow-xl"
            />
          )}
          <p className="text-lg font-semibold mt-5">{viewing?.name}</p>
          {viewing?.phone && (
            <p className="text-sm text-muted-foreground tabular-nums mt-0.5">{viewing.phone}</p>
          )}
          {viewing?.phone && (
            <Button asChild className="w-full rounded-xl h-11 font-semibold mt-5">
              <a href={telHref(viewing.phone)}>
                <Phone className="size-4 mr-1.5" /> Call {viewing.name?.split(" ")[0] || "now"}
              </a>
            </Button>
          )}
        </DialogContent>
