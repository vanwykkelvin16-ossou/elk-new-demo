...
function AdminVouchersPage() {
  ...
  const save = useMutation({
    mutationFn: async (e: EditState) => {
      ...
      // `legacy` holds the long-standing columns; `payload` adds the newer
      // business tel/address. If those columns haven't reached the live DB
      // yet, the write is retried with `legacy` so saving never hard-fails.
      const legacy = {
        title: (e.title ?? "").trim(),
      ...
      const isNew = !e.id;
      const write = async (body: typeof legacy | typeof payload) =>
        e.id
          ? supabase.from("vouchers").update(body).eq("id", e.id)
          : supabase.from("vouchers").insert(body);
