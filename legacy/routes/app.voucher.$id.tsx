  ...
  const { data: claim, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("voucher_claims")
        .select(
          // Select all voucher columns rather than naming them: if a newer
          // column (e.g. business_phone/address) hasn't reached the live DB
          // yet, naming it would fail the whole query and break this page.
          "id, voucher_id, claimed_at, expires_at, redeemed_at, vouchers (*)",
        )
        .eq("id", id)
        .eq("user_id", user!.id)
        .maybeSingle();
