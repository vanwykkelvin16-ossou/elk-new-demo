...
function AdminMembersPage() {
  ...
  // Member whose admin role is about to be granted/revoked (confirm first).
  const [roleChange, setRoleChange] = useState<Row | null>(null);
    ...
    <div className="space-y-6">
        ...
        <DialogContent className="max-w-lg sm:max-w-2xl max-h-[92vh] flex flex-col gap-0 p-0 overflow-hidden">
          ...
          {viewing && (
            <ModalBody>
              ...
              {/* Role & access — promote a member to admin or demote back */}
                ...
                <div className="rounded-xl border border-border bg-muted/20 p-4 flex items-center gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold">
                      {viewing.isAdmin ? "Administrator" : "Member"}
                    </p>
                    <p className="text-[11px] text-muted-foreground mt-0.5 leading-relaxed">
                      {viewing.isAdmin
                        ? "Has full access to the admin portal."
                        : "Standard access — vouchers, events and shop only."}
                    </p>
                  </div>
                  {isMe ? (
      ...
      <AlertDialog open={!!roleChange} onOpenChange={(open) => !open && setRoleChange(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {roleChange?.isAdmin ? "Demote to member?" : "Promote to administrator?"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {roleChange?.isAdmin ? (
                <>
                  <span className="font-semibold text-foreground">
                    {roleChange ? memberName(roleChange) : ""}
                  </span>{" "}
                  will lose access to the admin portal and go back to a standard member account.
                </>
              ) : (
                <>
                  <span className="font-semibold text-foreground">
                  ...
                  will get full access to the admin portal — events, vouchers, shop, customers and
                  contact info. You can demote them again at any time.
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={(e) => {
                e.preventDefault();
                if (roleChange) {
                  setRole.mutate({ userId: roleChange.id, grant: !roleChange.isAdmin });
                  setRoleChange(null);
                }
              }}
              disabled={setRole.isPending}
            >
              ...
              {roleChange?.isAdmin ? "Demote" : "Promote to admin"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
